...
# Assign data flow colors for the directions
$ns color 1 Green
$ns color 2 Red
...
proc record_bandwidth {} {
    ...
    set bandwidth1 [$sink_agent1 set bytes_]
    set bandwidth2 [$sink_agent2 set bytes_]

    # Record the bandwidth in Mbps and write to output files
    puts $output_file1 "$now [expr $bandwidth1/$time_interval*8/1000000]"

    puts $output_file2 "$now [expr $bandwidth2/$time_interval*8/1000000]"

    # Calculate throughput after 100 seconds
    if { $now >= 100 } {
        set throughput1 [expr $throughput1+ $bandwidth1/$time_interval*8/1000000 ]
        set throughput2 [expr $throughput2+ $bandwidth2/$time_interval*8/1000000 ]
    }
    ...
    $ns at [expr $now + $time_interval] "record_bandwidth"
}
